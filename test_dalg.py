...
DALG_GOLDEN_DIR  = f"{GOLDEN_DIR}dalg/" 
...
def check_dalg(answer, ckt, fault):
    """Using fd_csv"""
    passed=True
...
def generate_tests():
    ...
    each cmd file:
        ...
        DALG fault_num stuck_val output
    ...
    for ckt in os.listdir(CIRCUIT_DIR):
        ...
        fault_list = open(f'./auto-tests-phase3/inputs/_dalg/_{ckt}_dalg_faults.txt','r').readlines()
        idx = 0
        for fault in fault_list:
            fout = f'{DALG_CMD_TESTS_DIR}{ckt}_dalg_{idx}.txt'
            with open(fout, 'w') as ftest:
                f_num, s_val = list(fault.strip().split(' '))
                ftest.write(f'READ ../{CIRCUIT_DIR}{ckt}.ckt\n')

                ftest.write(f'DALG {f_num} {s_val} ../{DALG_OUTPUT_DIR}{ckt}-DALG-{f_num}-{s_val}.txt\n')

                ftest.write('QUIT')
            output_files.append(f'./{DALG_OUTPUT_DIR}{ckt}-DALG-{f_num}-{s_val}.txt')
            cmd_tests_files.append(f'{ckt}_dalg_{idx}.txt')
...
if __name__ == '__main__':
    ...
    for i in range(len(cmd_test_files)):
        ...
        try:
            gold_fail = False
            print('_'*50)
            ckt = cmd_test.replace('.txt','').replace('_dalg','').split('_')[0]
            print('Testing DALG on ' + cmd_test.replace('.txt', ''))
            os.system(f"cd build/; ./simulator < ../{DALG_CMD_TESTS_DIR}{cmd_test} > /dev/null ")
            
            fault_str=student_answer.replace('.txt','').split('DALG-')[-1].replace('-','@')

            if check_dalg(student_answer, ckt, fault_str):
